...
def get_logfile_path(directory, logfile):
    return logfile if os.path.isabs(logfile) else directory + '/' + logfile

def create_logfile(abs_logfile_path): 
    os.makedirs(os.path.split(abs_logfile_path)[0], exist_ok=True)
    open(abs_logfile_path, 'w').close()

def log_file_path(name, conf):
    if not conf.has_section(name):
        print_err_and_exit(instance_not_found % name, 1)
    if not conf.has_option(name, 'logfile') or conf.get(name, 'logfile') == '-':
        print_err_and_exit(loggin_not_configured % name, 1)
    logfile_path = get_logfile_path(conf.get(name, 'chdir'), conf.get(name, 'logfile'))
